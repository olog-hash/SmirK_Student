...
- **DistanceSearch (Ромбовидный послойный поиск)** - обходит клетки ромбовидными слоями на основе Манхэттенского расстояния (все клетки на расстояния от точки на 1, затем 2, 3, и т.д.). Ключевая оптимизация - вместо проверки всех клеток слоя, вычисляются граничные точки (вершины ромба), по которым строятся диагонали с автоматическим обрезанием за границами карты. Оптимален для карт среднего размера с средним распределением водителей.
